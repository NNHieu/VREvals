...
def run_evaluation(filtered_data, input_list, output_list, dataset_name, output_dir, total_time, split, apply_backoff=False, k_list=[1, 5, 10, 25, 50, 100]):
    ...
    if dataset_name == 'livecode':
        ...
        for item, input_prompt, results in zip(filtered_data, input_list, output_list):
            if type(results) == str:
                results = [results]
                # item['Output'] = result
            else:
                results = [results.outputs[i].text for i in range(results.outputs)]
            ...
            for result in results:
                ...
                generations_list[-1].append(pred_code)
                item['Pred_Answer'].append(pred_code)

        # Call codegen_metrics with pass@1
        metrics, results, final_metadata = codegen_metrics(
            samples_list,
            generations_list,
            k_list=[1, 2],  # Evaluate the top 1 generated result
        ...
        for item, input_prompt, results in zip(filtered_data, input_list, output_list):
            ...
            #     results = [results]
            # else:
            #     results = [results.outputs[i].text for i in range(len(results.outputs))]

            if type(results) != SamplerResponse:
                raise ValueError
            results = [choice.response_text for choice in results.choices]          
            if dataset_name in ['gpqa', 'medmcqa']:
                labeled_answer = item["Correct Choice"]
                # labeled_choice_answer = item["Correct Answer"]
                mode = 'choose'
            elif dataset_name in ['math500', 'aime', 'amc', 'simp_math', 'omega_func_area', 'omega_func_area_out', 'gsm8k']:
            ...
            # Assume multiple generation
            item['Output'] = []
            item['Question'] = input_prompt
            for gen in results:
                metric, pred_answer = evaluate_predictions(output=gen, labeled_answer=labeled_answer, mode=mode)
                ...
                item['Output'].append(gen_eval_result)
            
            item['num_samples'] = len(item['Output'])
            item['num_math_equal'] = sum([eval_result.metric['math_equal'] for eval_result in item['Output']])

        # compute pass@k using math_equal
        num_samples = np.array([item['num_samples'] for item in filtered_data])
        num_correct = np.array([item['num_math_equal'] for item in filtered_data])
        detail_pass_at_k = {
    ...
    with open(os.path.join(output_dir, result_json_name), mode='w', encoding='utf-8') as json_file:
        json.dump(filtered_data, json_file, indent=4, ensure_ascii=False, default=pydantic_encoder)
